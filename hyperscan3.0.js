...
function szdsq(delay) {
  ...
  cal();
  a = window.setInterval(cal, delay);
  return a;
}
//格式化函数
var intervalId;
...
function cal() {
  ...
  if (xmlHttp != null) {
    ...
    xmlHttp.open("get", "label.csv", true);
    xmlHttp.send();
  ...
  async function doResult() {
    if (xmlHttp.readyState == 4) {
      ...
      for (var s = 0; s < xs / p; s++) {
        async function pdzy(a, num, flag, f1, canvas, s1, ngn) {
          global[a] = global[a] + 0.5 * num[s];
          canvasScore(
            canvas,
            globalo[s1],
        ...
        pdzy("a", num1, "flag1", "f1", "Canvas01", "s1", "单脑A");
        pdzy("b", num2, "flag2", "f2", "Canvas02", "s2", "单脑B");
        pdzy("c", num3, "flag3", "f3", "Canvas03", "s3", "双脑");
        await sleep(1000 * p);
        var t2 = new Date();
        t3 = t2 - t1;
        //判断三个框是否执行完毕
