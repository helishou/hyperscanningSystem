  ...
  async function doResult() {
    if (xmlHttp.readyState == 4) {
      ...
      for (i = 0; i < l; i++) {
        ...
        if (label == 1) {
          document.getElementById("resText").innerHTML = '  Trial  ' + i + '   想像:向右';
        }
        else if (label == 0) {
          document.getElementById("resText").innerHTML = '  Trial  ' + i + '   想像:空闲';
        }
        else {
          document.getElementById("resText").innerHTML = '  Trial  ' + i + '   想像:向左';
        }
        var t1 = +new Date();
        if (data1[1] == getMaxValue(data1[0], data1[1], data1[2]) && data1[3] == 0) {
          num1 = randomArr(trialtime / p, -5, 5);
        }
        else {
          num1 = randomArr(trialtime / p, data1[0] - data1[2] + 2 * data1[3] - 15, data1[0] - data1[2] + 2 * data1[3] + 15);
        }
        if (data2[1] == getMaxValue(data2[0], data2[1], data2[2]) && data2[3] == 0) {
          num2 = randomArr(trialtime / p, -5, 5);
        }
        else {
          num2 = randomArr(trialtime / p, data2[0] - data2[2] + 2 * data2[3] - 15, data2[0] - data2[2] + 2 * data2[3] + 15);
        }
        if (data3[1] == getMaxValue(data3[0], data3[1], data3[2]) && data3[3] == 0) {
          num3 = randomArr(trialtime / p, -5, 5);
        }
        else {
          num3 = randomArr(trialtime / p, data3[0] - data3[2] + 2 * data3[3] - 15, data3[0] - data3[2] + 2 * data3[3] + 15);
        }
        // alert(num1)
        for (var s = 0; s < (xs / p); s++) {
          async function pdzy(a, num, data, flag, f1, canvas, s1, ngn) {
            global[a] = global[a] + 1*num[s]
            canvasScore(canvas, globalo[s1], (globalo[s1] / j).toFixed(2), global[a], ngn + "：判断中");
            if (global[a] <= ScaleMinValue || global[a] >= ScaleMaxValue) {
              if (global[a] <= ScaleMinValue) {
                global[a]=global[a]-500;
                global[flag] = 1;
                //终止定时器
                canvasScore(canvas, globalo[s1], (globalo[s1] / j).toFixed(2), ScaleMinValue, ngn + "：判断中");
                if (parseInt(data[3]) == label) {
                  global[f1] = 1;
                  canvasScore(canvas, globalo[s1], (globalo[s1] / j).toFixed(2), ScaleMinValue, ngn + "：正确");
                }
                else {
                  canvasScore(canvas, globalo[s1], (globalo[s1] / j).toFixed(2), ScaleMinValue, ngn + "：错误");
                }
              }
              if (global[a] >= ScaleMaxValue) {
                ...
                global[flag] = 1;
                //终止定时器
                canvasScore(canvas, globalo[s1], (globalo[s1] / j).toFixed(2), ScaleMaxValue,  ngn + "：判断中");
                if (parseInt(data[3]) == label) {
                  global[f1] = 1;
                  canvasScore(canvas, globalo[s1], (globalo[s1] / j).toFixed(2), ScaleMaxValue, ngn + "：正确");
                }
                else {
                  canvasScore(canvas, globalo[s1], (globalo[s1] / j).toFixed(2), ScaleMaxValue, ngn + "：错误");
                }
              }
            }
          }
          pdzy('a', num1, data1, 'flag1', 'f1', "Canvas01", 's1', "单脑A")
          pdzy('b', num2, data2, 'flag2', 'f2', "Canvas02", 's2', "单脑B")
          pdzy('c', num3, data3, 'flag3', 'f3', "Canvas03", 's3', "双脑")
          await sleep(1000 * p);
          var t2 = new Date();
          t3 = (t2 - t1) / 1000;
          if (global['flag1'] + global['flag2'] + global['flag3'] == 3 || t3 >= xs) {
            break
          }
        }
        async function pdkx(flag, f, data, canvas, s, ngn) {
          if (global[flag] == 0 && parseInt(data[3]) == label) {
            global[f] = 1;
            canvasScore(canvas, globalo[s], (globalo[s] / j).toFixed(2), 240, ngn + "：正确");
          }
          else {
            canvasScore(canvas, globalo[s], (globalo[s] / j).toFixed(2), 240, ngn + "：错误");
          }
        }
        pdkx('flag1', 'f1', data1, "Canvas01", 's1', "单脑A")
        pdkx('flag2', 'f2', data2, "Canvas02", 's2', "单脑B")
        pdkx('flag3', 'f3', data3, "Canvas03", 's3', "双脑")
        if(global['f1']==1){
        ...
        global['f3'] = 0;
        j = j + number;
        refresh(globalo['s1'], globalo['s2'], globalo['s3'],(globalo['s1'] / j).toFixed(2),(globalo['s2'] / j).toFixed(2),
        (globalo['s3'] / j).toFixed(2),240);      
        await sleep((5500 - t3) );
      }
    }
  }
